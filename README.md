...
> 4. Set your own `USERS` env var for login (see [Access & Auth](#access--auth))
>
...
- Itemized per-line detail still isn't saved to History (Known Blocker #23, unchanged)
...
- Password changes: on the live deployment, that requires access to Gian's Vercel account — ask Sir Tony or Engr. Emir if that's ever needed. On your own fork, you set your own `USERS` env var directly (Settings → Environment Variables), no one to ask.
