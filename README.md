...
26. **"Pallet Size" input (Capacity tab, calculator.html) is now orphaned.** Found while fixing the Isle Width bug (see this session's Status entry) — the correct formula only ever needed Pallet Depth + Isle Width, "Pallet Size" was never actually part of it. Flagged, not removed — decide whether to delete the field or repurpose it.
27. **"No. of Stories" still doesn't factor into Building Volume**, and its relevance for warehouse-type buildings (typically single-story with a mezzanine, not literally multiple floors) was raised but not resolved this session. Options on the table: leave as-is, remove the field entirely, or wire it into the formula properly if a real multi-story case comes up. Needs a decision, not more investigation.
